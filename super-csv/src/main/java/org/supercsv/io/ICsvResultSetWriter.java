...
public interface ICsvResultSetWriter extends ICsvWriter {
	 ...
	 * corresponds to a column in {@code ResultSet}, column order is preserved.
	 * Column names in CSV file corresponds to column names stored in
	 * {@code ResultSetMetaData}. 
	 * 
	 * {@code toString} will be called on each element prior to writing.
	 * 
	 * @param 	resultSet		
	 ...
	 * {@code ResultSetMetaData}.
	 * 
	 * {@code toString} will be called on each (processed) element prior to writing.
	 * 
	 * @param 	resultSet		
	 * 				ResultSet containing the values to write
	 * @param	cellProcessors
	 * 				Array of CellProcessors used to further process data before 
	 * 				it is written (each element in the processors array corresponds 
	 * 				with a CSV column - the number of processors should match the number of
	 *            	columns). A {@code null} entry indicates no further 
	 *            	processing is required (the value returned by
