...
public class TrimTest {
	...
	public void chainTest() throws Exception {
		...
		org.junit.Assert.assertEquals("trim and chain", true,
			new Trim(3, cpp).execute("fooo", TestConstants.ANONYMOUS_CSVCONTEXT));
		org.junit.Assert.assertEquals("trim and chain", true,
			new Trim(3, "", cpp).execute("fooo", TestConstants.ANONYMOUS_CSVCONTEXT));
	}
	...
	public void trimTest() throws Exception {
		org.junit.Assert.assertEquals("no trim = max size", "foo",
			cp.execute("foo", TestConstants.ANONYMOUS_CSVCONTEXT));
		org.junit.Assert.assertEquals("no trim < max size", "fo", cp.execute("fo", TestConstants.ANONYMOUS_CSVCONTEXT));
		org.junit.Assert.assertEquals("trim", "foo", cp.execute("fooo", TestConstants.ANONYMOUS_CSVCONTEXT));
		
		cp = new Trim(3, "..");
		org.junit.Assert.assertEquals("no trim = max size", "foo",
			cp.execute("foo", TestConstants.ANONYMOUS_CSVCONTEXT));
		org.junit.Assert.assertEquals("trim and append", "foo..",
			cp.execute("fooo", TestConstants.ANONYMOUS_CSVCONTEXT));
	}
