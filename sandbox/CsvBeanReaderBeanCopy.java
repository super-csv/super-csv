 ...
 */
public class CsvBeanReaderBeanCopy extends AbstractCsvReader implements ICsvBeanReader {
/**
 ...
 */
public CsvBeanReaderBeanCopy(final Reader reader, final CsvPreference preferences) {
...
<T> T fillObject(final Class<T> clazz, final String[] nameMapping) throws SuperCSVReflectionException {
	try {
		...
		for( int i = 0; i < nameMapping.length; i++ ) {
			...
			try {
				BeanUtils.setProperty(resultBean, nameMapping[i], lineResult.get(i));
			}
