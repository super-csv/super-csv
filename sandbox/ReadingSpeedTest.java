...
import org.supercsv.io.CsvBeanReaderBeanCopy;
...
private void readCsvBeanFullBeanUtilsReader(final Reader r) throws Exception {
	final CsvBeanReaderBeanCopy csv = new CsvBeanReaderBeanCopy(r, CsvPreference.EXCEL_PREFERENCE);
...
public void readSpeedTest() throws Exception {
	...
	for( int i = 0; i < TEST_RUNS; i++ ) {
		...
		t = System.currentTimeMillis();
		readCsvBeanFullBeanUtilsReader(new FileReader(TEST_FILE));
		times[testNo++] += System.currentTimeMillis() - t;
	}
	{
		int i = 0;
		...
		System.out.println(makeTableLine("BeanReaderBeanUtil (full)", times[0], times[i++]));
