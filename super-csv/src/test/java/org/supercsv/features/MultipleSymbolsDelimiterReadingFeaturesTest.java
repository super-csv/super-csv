...
public class MultipleSymbolsDelimiterReadingFeaturesTest {
	...
	private CsvPreference multipleDelimiterPreference = new Builder('"', "+,-", "\r\n").build();

	@Test
	public void testCustomSeparator() throws IOException {
		String csv = "John+,-Connor+,Sarah";
		...
		Assert.assertEquals("Connor+,Sarah", result.get(1));
	}
	...
	public void testGetByColumnIndex() throws IOException {
		...
		CsvListReader listReader = new CsvListReader(new StringReader(csv), multipleDelimiterPreference);
	...
	public void testGetByColumnName() throws IOException {
		...
		CsvMapReader mapReader = new CsvMapReader(new StringReader(csv), multipleDelimiterPreference);
	...
	public void testSkipCommentLines() throws IOException {
		...
		CsvPreference customPreference = new Builder(multipleDelimiterPreference).skipComments(new CommentMatches("<!--.*-->"))
			.build();
		CsvListReader listReader = new CsvListReader(new StringReader(csv), customPreference);
		List<String> line = listReader.read();
	...
	public void testColumnNameBasedMapping() throws IOException {
		...
		CsvBeanReader beanReader = new CsvBeanReader(new StringReader(csv), multipleDelimiterPreference);
